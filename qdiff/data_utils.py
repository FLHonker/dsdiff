...
from qdiff.quant_layer import QuantModule, Union
...
def save_inp_oup_data(model: QuantModel, layer: Union[QuantModule, BaseQuantBlock], cali_data: torch.Tensor,
    ...
    for i in range(int(cali_data[0].size(0) / batch_size)):
        cur_inp, cur_out = get_inp_out([x[i * batch_size : (i + 1) * batch_size] for x in cali_data])
        cached_batches.append((cur_inp, cur_out.cpu()))
    
    if isinstance(cur_inp, list):
        cached_inps = [torch.cat([x[0][i] for x in cached_batches]) for i in range(len(cur_inp))]
    else:
...
class GetLayerInpOut:
    ...
    def __call__(self, model_input):
        ...
        with torch.no_grad():
            try:
                _ = self.model(*[x.to(self.device) for x in model_input]) # input tuple
            ...
            if self.asym:
                ...
                try:
                    _ = self.model(*[x.to(self.device) for x in model_input])
                except StopForwardException:
                    pass
        ...
        # modified to QuantResBlock input
        input_store = self.data_saver.input_store
        if len(input_store) > 1:
            input_store = [x.detach().cpu() for x in input_store]
        else:
            input_store = input_store[0].detach().cpu()

        return input_store, self.data_saver.output_store.detach()
